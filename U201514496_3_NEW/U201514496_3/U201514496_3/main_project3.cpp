...
int main(int agrc,char* agrv[]) {
	...
	char filename[20] = "U201514496_3.txt";
	...
	while (num_cnt < agrc) {
		if (!strcmp("-I", agrv[num_cnt])) {
			...
			while ((strcmp("-I", agrv[num_cnt]) && strcmp("-O", agrv[num_cnt]) && strcmp("-A", agrv[num_cnt]) && strcmp("-C", agrv[num_cnt]) && strcmp("-N", agrv[num_cnt]) && strcmp("-G", agrv[num_cnt])) && (num_cnt < agrc - 1)) {
				...
				if (pstack->size() <= pstack->operator int()) {
					cout << "errors" << endl;
					fprintf(fp, "%c", 'E');
					fclose(fp);
					return 0;
				}
				else pstack->operator<<(get_num);
				++num_cnt;
			}
			if (num_cnt == agrc - 1) {
				get_num = atoi(agrv[num_cnt]);
			...
			fprintf(fp, "%c  ", 'N');
			fprintf(fp, "%d  ", pstack->operator int());
			...
			++num_cnt;
			get_num = atoi(agrv[num_cnt]);
			if (get_num < pstack->operator int()) {
				fprintf(fp, "%d  ", pstack->operator[](get_num));
			}
			else {
				cout << "Errors!" << endl;
				fprintf(fp, "%c", 'E');
			...
			for (; get_num > 0; get_num--) {
				if (pstack->operator int() <= 0) {
					cout << "Errors" << endl;
					fprintf(fp, "%c  ", 'E');
					fclose(fp);
					return 0;
				}
				else pstack->operator>>(lost_num);
			}
			pstack->print(fp);
			++num_cnt;
		}
